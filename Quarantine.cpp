...
QuarantineManager::~QuarantineManager() { if (m_db) sqlite3_close(m_db); }
...
bool QuarantineManager::initialize(const std::string& dbPath) {
    ...
    if (rc != SQLITE_OK) {
        std::cerr << "Erro ao abrir banco: " << sqlite3_errmsg(m_db) << std::endl;
        return false;
    }
    return createTables();
}
...
bool QuarantineManager::createTables() {
    ...
    if (rc != SQLITE_OK) {
        std::cerr << "Erro SQL: " << err << std::endl;
        sqlite3_free(err);
        return false;
    }
    return true;
...
bool QuarantineManager::quarantineFile(const std::string& originalPath, const std::string& threatName) {
    try {
        // Gera nome único para o arquivo em quarentena
        ...
        // Move o arquivo para quarentena (copia + remove)
        ...
        std::cerr << "Erro em quarentena: " << e.what() << std::endl;
        return false;
    }
}
...
bool QuarantineManager::restoreFile(int id) {
    ...
    for (auto& e : entries) {
        if (e.id == id) {
            try {
                fs::copy(e.quarantinedPath, e.originalPath, fs::copy_options::overwrite_existing);
                ...
                std::cerr << "Erro ao restaurar: " << ex.what() << std::endl;
                return false;
            }
        }
    }
...
std::vector<QuarantineEntry> QuarantineManager::listAll() {
    ...
    sqlite3_stmt* stmt;
    const char* sql = "SELECT id, original_path, quarantined_path, threat_name, date FROM quarantine ORDER BY id DESC;";
